# ds4-b — DS-4 summaries + theme wiki drafts

Working root: C:\dev\GeoBase\docs\data-sharing-agreements. Your lane:
`ds4-b` (lanes.json). Contract: prompts/codex-ds4-summarize.md in full
...
summaries/<doc-id>.md, wiki/<theme>.ds4-b.md, provenance/access-log.ds4-b.jsonl,
event ids ev-ds4-b-NNNN, real UTC ts).
...
## Assigned summaries (one page each; doc list = prompts/_ds4-b-docs.txt)
...
## Assigned wiki theme drafts (wiki/<theme>.ds4-b.md)

- publication-approval — pre-publication review/approval rights
- benefit-sharing-data-return — benefit sharing + data return/destruction
- enforcement-jurisdiction — enforcement, remedies, jurisdiction, sovereign immunity boundaries
- ip-tk-protection — intellectual property + Traditional Knowledge protections
...
Also note in your enforcement-jurisdiction draft the ds3-d theme finding
that government-to-government process duties (RCW 43.376) are a separate
control plane from artifact classification.

Finish: `python tools/merge_validate.py validate` clean; lane report
reviews/lane-reports/ds4-b-2026-07-21.md (docs covered, claims you were
